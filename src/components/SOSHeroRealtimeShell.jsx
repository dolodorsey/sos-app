...
export default function SOSHeroRealtimeShell(){
 ...
 useEffect(()=>{const s=session();if(!s?.access_token)return;const client=createClient(SB,SK,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}});client.realtime.setAuth(s.access_token);
   const notifyOffer=payload=>{if(document.hidden&&'Notification'in window&&Notification.permission==='granted'){try{new Notification('New S.O.S. mission offer',{body:'Open Hero Command to review the live offer.'})}catch{}}refresh('Offers')};
   const channel=client.channel(`sos-hero-live-${s.user.id}`)
   ...
   return()=>{if(timer.current)clearTimeout(timer.current);client.removeChannel(channel)};
 },[]);
