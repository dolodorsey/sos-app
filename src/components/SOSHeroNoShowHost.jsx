...
export default function SOSHeroNoShowHost(){
 ...
 const settle=async()=>{const s=stored();if(!s||!mission||!quote?.canSettle||busy)return;if(!window.confirm(`Close this mission as a customer no-show?\n\nS.O.S. will capture a $${Number(quote.feeAmount||0).toFixed(2)} no-show fee. Your Hero compensation is $${Number(quote.heroCompensation||0).toFixed(2)}.`))return;setBusy(true);setNotice('');try{const d=await request('/functions/v1/sos-customer-no-show',{method:'POST',token:s.access_token,body:{missionId:mission.id,action:'settle',expectedFeeAmount:Number(quote.feeAmount||0)}});setMission(null);setQuote(null);setNotice(`Customer no-show recorded. $${Number(d.heroCompensation||0).toFixed(2)} Hero compensation is processing.`)}catch(e){setNotice(e.message)}finally{setBusy(false);setTimeout(()=>setNotice(''),5000)}};
 if(!mission&&!notice)return null;const remaining=Math.max(0,Math.ceil(Number(quote?.secondsRemaining||0)));
 return <div style={{position:'fixed',left:18,bottom:190,zIndex:1480,display:'grid',gap:7}}>
  {mission&&quote&&!quote.canSettle&&<div style={{padding:'10px 13px',borderRadius:999,background:'rgba(17,27,37,.96)',color:'#fff',border:'1px solid rgba(255,255,255,.12)',fontSize:10,fontWeight:900,boxShadow:'0 12px 36px rgba(0,0,0,.32)'}}>CUSTOMER NO-SHOW · {clock(remaining)}</div>}
