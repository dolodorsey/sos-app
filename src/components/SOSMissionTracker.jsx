...
export default function SOSMissionTracker({token,missionId,initialMission,onClose}){
  ...
  const mapUrl=useMemo(()=>{
    const heroLive=['assigned','en_route','on_site','working'].includes(phase)&&livePosition?.lat!=null&&livePosition?.lng!=null;
    const lat=heroLive?livePosition.lat:(mission?.pickup_lat||initialMission?.pickup_lat||33.749);
    const lng=heroLive?livePosition.lng:(mission?.pickup_lng||initialMission?.pickup_lng||-84.388);
    const dx=.035,dy=.025;
  ...
  },[phase,livePosition?.lat,livePosition?.lng,mission?.pickup_lat,mission?.pickup_lng,initialMission?.pickup_lat,initialMission?.pickup_lng]);
  ...
  useEffect(()=>{
    ...
    const channel=client.channel(`sos-customer-mission-${missionId}`)
      ...
      .on('postgres_changes',{event:'*',schema:'public',table:'sos_payments',filter:`mission_id=eq.${missionId}`},refresh)
      .on('postgres_changes',{event:'*',schema:'public',table:'sos_mission_live_positions',filter:`mission_id=eq.${missionId}`},payload=>{
        const row=payload.new||payload.old||{};const lat=Number(row.lat),lng=Number(row.lng);
        if(active&&Number.isFinite(lat)&&Number.isFinite(lng))setLivePosition({lat,lng,updatedAt:row.updated_at||new Date().toISOString()});
      })
      .subscribe(status=>{if(active)setRealtimeState(status==='SUBSCRIBED'?'live':status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED'?'fallback':'connecting')});
    client.from('sos_mission_live_positions').select('lat,lng,updated_at').eq('mission_id',missionId).maybeSingle().then(({data})=>{if(active&&data)setLivePosition({lat:Number(data.lat),lng:Number(data.lng),updatedAt:data.updated_at})});
  ...
  return <div className="sos-mobility-layer" role="dialog" aria-modal="true" aria-label="S.O.S. mission tracker">
    <div className="sos-live-map"><iframe title="S.O.S. live mission map" src={mapUrl}/><div className="sos-map-overlay"/><div className="sos-radar"><span/><i/><b/></div><div className="sos-live-pill"><span/> {livePosition&&['assigned','en_route','on_site','working'].includes(phase)?'HERO LIVE':realtimeState==='live'?'LIVE MISSION':'LIVE MISSION · POLLING'}</div></div>
