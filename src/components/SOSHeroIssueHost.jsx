...
export default function SOSHeroIssueHost(){
 ...
 const report=async()=>{const s=stored();if(!s||!mission||busy)return;const reason=window.prompt('Issue type: service_incomplete, damage_claim, wrong_service, pricing_dispute, safety_incident, or other','other')?.trim();if(!reason)return;const description=window.prompt('Briefly describe what happened:')?.trim();if(!description)return;setBusy('report');setNotice('');try{const id=await request('/rest/v1/rpc/sos_open_mission_dispute',{method:'POST',token:s.access_token,body:{p_mission_id:mission.id,p_reason:reason,p_description:description}});setNotice(`Case ${String(id).replaceAll('"','').slice(0,8).toUpperCase()} opened.`)}catch(e){setNotice(e.message)}finally{setBusy('');setTimeout(()=>setNotice(''),5000)}};
