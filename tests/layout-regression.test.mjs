...
test('visible SOS profile controls are direct and backed by real account operations', () => {
  ...
  for (const label of ['Vehicles','Payment methods','Shield membership','Safety & support','Become an S.O.S. Hero']) assert.match(customer,new RegExp(label.replace(/[&]/g,'\\&')))
  ...
  assert.match(tools,/sos:vehicles-changed/)
