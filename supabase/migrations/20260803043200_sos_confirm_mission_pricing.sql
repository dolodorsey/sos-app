...
begin
  ...
  insert into public.sos_mission_events(mission_id,event_type,old_status,new_status,payload,actor) values(p_mission_id,'pricing_confirmed',v_mission.status,v_mission.status,jsonb_build_object('final_price',v_mission.final_price,'operator_auth_id',p_operator_auth_id),'operator');
  return v_mission;
end $$;
