...
Deno.serve(async(req)=>{
  ...
  const {data:mission}=await admin.from('sos_missions').select('id,hero_id,status,subcategory_id,estimated_price,pricing_status').eq('id',missionId).single()
  if(!mission||mission.hero_id!==hero.id)return json(req,{error:'Assigned mission not found'},404)
  if(!['assigned'].includes(mission.status))return json(req,{error:'Final price must be confirmed before the Hero starts the route'},409)
  ...
  const {data:updated,error}=await admin.from('sos_missions').update({final_price:rounded,pricing_status:'confirmed',updated_at:now}).eq('id',mission.id).eq('hero_id',hero.id).eq('status','assigned').select('id,final_price,pricing_status,status').single()
  if(error||!updated)return json(req,{error:error?.message||'Price could not be confirmed'},409)
  await admin.from('sos_mission_events').insert({mission_id:mission.id,event_type:'pricing_confirmed',old_status:mission.status,new_status:mission.status,payload:{final_price:rounded,service_id:serviceRow.id,price_model:serviceRow.price_model,source:'hero_app'},actor:'hero'})
